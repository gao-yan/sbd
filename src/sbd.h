...
extern bool             timeout_watchdog_warn_is_default;
...
#define calculate_timeout_watchdog_warn(timeout_watchdog) \
	(timeout_watchdog < 5 ? 2 : \
	(timeout_watchdog * 3 <= INT32_MAX ? \
	(timeout_watchdog * 3 / 5) : (timeout_watchdog / 5 * 3)))
